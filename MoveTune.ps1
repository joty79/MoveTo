...
function Show-Config {
    ...
    Write-Host ""
    Write-Host "Current MoveTune config" -ForegroundColor Cyan
...
function Show-HowToUse {
    ...
    Write-Host "8. Install MoveTo" -ForegroundColor Gray
    ...
    Write-Host "   - Opens C:\Users\joty79\AppData\Local\MoveToContext." -ForegroundColor Gray
    Write-Host ""
...
function Clear-RoboLogs {
    $candidateLogDirs = @(
        ...
        (Join-Path $env:LOCALAPPDATA "MoveToContext\logs")
...
function Launch-MoveToInstaller {
    $installerPath = 'C:\Users\joty79\AppData\Local\MoveToContext\Install.ps1'
    if (-not (Test-Path -LiteralPath $installerPath)) {
        ...
        Write-Host "[2] Local source (D:\Users\joty79\scripts\MoveTo)" -ForegroundColor Gray
        Write-Host "[0] Cancel" -ForegroundColor Gray
        $choice = (Read-Host "Select option").Trim()

        switch ($choice) {
            '1' {
                $url = 'https://raw.githubusercontent.com/joty79/MoveTo/master/Install.ps1'
                try {
                    Invoke-WebRequest -Uri $url -Headers @{ 'User-Agent' = 'MoveTune/1.0' } -UseBasicParsing -OutFile $installerPath
            ...
            '2' {
                $localSource = 'D:\Users\joty79\scripts\MoveTo\Install.ps1'
...
function Open-MoveToInstallDirectory {
    $installRoot = 'C:\Users\joty79\AppData\Local\MoveToContext'
    if (-not (Test-Path -LiteralPath $installRoot)) {
...
while ($true) {
    ...
    Write-Host ""
    Write-Host "=== MoveTune Menu ===" -ForegroundColor Green
    ...
    Write-MenuLine -Number "8" -Prefix "" -Highlight "Install MoveTo" -Suffix "" -HighlightColor Cyan
    ...
    switch ($choice) {
        ...
        "2" {
            Write-Host "=== Set Extra MoveTo " -NoNewline -ForegroundColor Gray
        ...
        "8" {
            Launch-MoveToInstaller
            Write-Host "Press any key to continue..." -ForegroundColor DarkCyan
            [Console]::ReadKey($true) | Out-Null
        }
        "9" {
            Open-MoveToInstallDirectory
            Write-Host "Press any key to continue..." -ForegroundColor DarkCyan
            [Console]::ReadKey($true) | Out-Null
        }
        default {
